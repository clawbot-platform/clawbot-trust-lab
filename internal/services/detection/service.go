...
func (s *Service) Rules() []detectionmodel.RuleDefinition {
	return []detectionmodel.RuleDefinition{
		...
		{ID: "missing_provenance_sensitive_action", Title: "Missing provenance on sensitive action", Description: "Sensitive actions should carry provenance evidence.", Severity: 15},
		...
		{ID: "repeat_suspicious_context", Title: "Replay and memory context suggest repeat suspicious behavior", Description: "Repeat suspicious context from replay and memory should elevate baseline concern.", Severity: 10},
	}
}
...
func (s *Service) buildContext(ctx context.Context, execution executionsvc.ExecutionResult) detectionmodel.DetectionContext {
	...
	delegatedActorPresent := refundRequestedByAgent || strings.Contains(order.SubmittedByActorID, "agent") || order.DelegationMode != "direct_human"
	...
	features := map[string]bool{
		...
		"refund_without_authorization": refundID != "" && (!approvalPresent || mandate.Status != "active"),
...
func (s *Service) evaluateRules(contextData detectionmodel.DetectionContext) []detectionmodel.RuleHit {
	...
	if (f["refund_requested"] || f["fully_delegated_action"]) && f["provenance_missing"] {
		hits = append(hits, detectionmodel.RuleHit{RuleID: "missing_provenance_sensitive_action", Title: "Missing provenance on sensitive action", Severity: 15, Reason: "sensitive action lacked provenance evidence", Metadata: map[string]any{"scenario_id": contextData.ScenarioID}})
	}
	...
	if f["refund_requested_by_agent"] && f["approval_missing"] {
	...
	if f["trust_decision_step_up"] && contextData.ReplayHistoryCount > 1 && contextData.MemoryContextPresent {
		hits = append(hits, detectionmodel.RuleHit{RuleID: "repeat_suspicious_context", Title: "Replay and memory context suggest repeat suspicious behavior", Severity: 10, Reason: "repeat suspicious context appears in replay history and memory records", Metadata: map[string]any{"replay_history_count": contextData.ReplayHistoryCount}})
	}
